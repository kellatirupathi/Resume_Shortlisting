...
#         # Display text inputs for each key - now supporting 10 keys
#         for i in range(10):
...
#             st.session_state.key_resume_counts = [0] * 10 # Reset counts for all 10 keys
...
# # ===== MODIFIED FUNCTION FOR COMPREHENSIVE DETAILS =====
# def process_comprehensive_details(row):
#     """
#     Processes a resume to extract comprehensive personal and educational details.
#     """
#     user_id = row['user_id']
#     resume_link = row['Resume link']
#     resume_link = correct_google_drive_url(resume_link)
...
#                 logging.error(f"Failed to retrieve URL: {resume_link}")
#     except Exception as e:
#         logging.error(f"Error retrieving resume for {user_id}: {e}")

#     if resume_pdf:
#         # Use the enhanced OCR-capable text extraction function
#         resume_text = extract_text_from_pdf(BytesIO(resume_pdf) if isinstance(resume_pdf, bytes) else resume_pdf)

#         # Construct the detailed prompt for all fields including Diploma, 12th, and 10th
#         comprehensive_prompt = f"""
# From the resume text provided below, extract the following information.
# Use the EXACT field names as specified. If a piece of information is not found, use blank or empty.

# Full Name:
# Mobile Number:
# Email Id:
# Masters/Doctorate Course Name:
# Masters/Doctorate Department Name:
...
# Diploma Name:
# Diploma Department Name:
# Diploma Year of Completion:
# Diploma Percentage:
# Intermediate / PUC / 12th Name:
# Intermediate / PUC / 12th Department Name:
...
# {resume_text}
# """
#         extracted_text = analyze_text_with_gemini(comprehensive_prompt)
        
#         # Parse the response from Gemini
#         details = {'User ID': user_id, 'Resume Link': resume_link}
        
#         # Define all fields we need to extract, including new ones
#         fields = [
#             "Full Name", "Mobile Number", "Email Id",
#             "Masters/Doctorate Course Name", "Masters/Doctorate Department Name",
#             "Masters/Doctorate Year of Completion", "Masters/Doctorate Percentage",
#             "Bachelors Course Name", "Bachelors Department Name",
#             "Bachelors Year of Completion", "Bachelors Percentage",
#             "Diploma Name", "Diploma Department Name", "Diploma Year of Completion", "Diploma Percentage",
#             "Intermediate / PUC / 12th Name", "Intermediate / PUC / 12th Department Name",
#             "Intermediate / PUC / 12th Year of Completion", "Intermediate / PUC / 12th Percentage",
#             "SSC / 10th Name", "SSC / 10th Year of Completion", "SSC / 10th Percentage"
#         ]
...
#         return details
#     else:
#         # Create a result with error messages if resume retrieval failed
#         error_details = {'User ID': user_id, 'Resume Link': resume_link}
#         fields = [
#             "Full Name", "Mobile Number", "Email Id",
#             "Masters/Doctorate Course Name", "Masters/Doctorate Department Name",
#             "Masters/Doctorate Year of Completion", "Masters/Doctorate Percentage",
...
#         ]
#         for field in fields:
#             error_details[field] = "Failed to process resume"
#         return error_details


# # ========== MODIFIED FUNCTION TO EXTRACT ONLY GITHUB PROFILE LINK ==========
# def process_resume_github_links(row):
#     """
#     Processes a resume to find and return a single, primary GitHub profile link.
#     It prioritizes direct profile links (e.g., github.com/username) and falls back
#     to constructing one from a repository link if necessary.
#     """
#     user_id = row['user_id']
#     resume_link = row['Resume link']
#     resume_link = correct_google_drive_url(resume_link)
...
#                 temp_pdf_file.write(resume_pdf if isinstance(resume_pdf, bytes) else resume_pdf.read())
...
#             text_found_links = re.findall(github_regex, resume_text)
#             embedded_github_links = [link for link in embedded_links if 'github.com' in link]
            
#             all_github_links = sorted(list(set(text_found_links + embedded_github_links)))
            
#             github_link_str = "No GitHub links found"
#             if all_github_links:
#                 profile_links = []
#                 # A profile URL path has one segment (the username), not more.
#                 for link in all_github_links:
#                     path = re.sub(r'https?://(?:www\.)?github\.com/', '', link).strip('/')
#                     if '/' not in path:
#                         profile_links.append(link)
                
#                 if profile_links:
#                     # Prioritize the first found direct profile link
#                     github_link_str = profile_links[0]
#                 else:
#                     # If only repo links found, build profile link from the first one
#                     first_repo_link = all_github_links[0]
#                     username = extract_github_username(first_repo_link)
#                     if username:
#                         github_link_str = f"https://github.com/{username}"
#                     else:
#                         github_link_str = "Profile link not found"

#             return {
#                 'User ID': user_id,
#                 'Resume Link': resume_link,
#                 'GitHub Links': github_link_str
#             }

#         except Exception as e:
#             logging.error(f"Error processing GitHub links for {user_id}: {str(e)}")
#             return {
#                 'User ID': user_id, 'Resume Link': resume_link, 'GitHub Links': 'Failed to process resume'
#             }
#     else:
#         return {
#             'User ID': user_id, 'Resume Link': resume_link, 'GitHub Links': 'Failed to retrieve resume'
#         }
...
#         'Mobile Number': mobile_number if mobile_number else '',
#         'Email ID': email_id if email_id else '',
#     }
...
#                     elif process_function == process_resume_github_links:
#                         error_result = {
#                             'User ID': row['user_id'],
#                             'Resume Link': row['Resume link'],
#                             'GitHub Links': 'Error processing resume'
#                         }
...
# # ===== NEW WRAPPER FUNCTION FOR COMPREHENSIVE DETAILS =====
# def extract_and_analyze_comprehensive_details(csv_file):
...
#     # Process resumes in batches using the new function
#     return process_resumes_in_batches(df, process_comprehensive_details, batch_size)


# # ========== NEW WRAPPER FOR GITHUB LINK EXTRACTION ==========
# def extract_and_analyze_github_links(csv_file):
#     """
#     Wrapper function to read a CSV and process resumes to extract GitHub links.
#     """
#     if isinstance(csv_file, (str, Path)):
#         df = pd.read_csv(csv_file)
#     else:
...
#     # Process resumes in batches using the new GitHub link extraction function
#     return process_resumes_in_batches(df, process_resume_github_links, batch_size)
...
#     elif 'GitHub Links' in df.columns:
#         processed_count = df[~df['GitHub Links'].str.contains('Failed', na=False)].shape[0]
#     else:
#         processed_count = 0
...
#             ["Select an option...", "Project Analysis", "Skills Analysis", "Personal Details Analysis", "Comprehensive Details", "GitHub Links Extraction"],
...
#             st.session_state.pop('comprehensive_details_results', None)
#             st.session_state.pop('github_links_results', None)
...
#         # ===== NEW UI BLOCK FOR COMPREHENSIVE DETAILS =====
#         elif analysis_type == "Comprehensive Details":
#             st.markdown("<h2>Comprehensive Details Extraction</h2>", unsafe_allow_html=True)

#             if 'comprehensive_details_results' not in st.session_state:
#                 with st.spinner('⏳ Extracting comprehensive details from resumes...'):
#                     if uploaded_file is not None:
#                         comp_details_df = extract_and_analyze_comprehensive_details(uploaded_file)
#                     else:
#                         with tempfile.NamedTemporaryFile(delete=False, suffix='.csv') as tmp:
#                             text_input_df.to_csv(tmp.name, index=False)
#                             tmp_path = tmp.name
                        
#                         comp_details_df = extract_and_analyze_comprehensive_details(tmp_path)
#                         os.unlink(tmp_path)
                    
#                     st.session_state['comprehensive_details_results'] = comp_details_df
            
#             comp_details_df = st.session_state['comprehensive_details_results']
            
#             # Display metrics
#             display_metrics(comp_details_df, "Comprehensive Details Results", "Total Candidates")
            
#             # Show the data
#             st.dataframe(comp_details_df, use_container_width=True)
            
#             # Separator
#             st.markdown("<hr style='margin: 1.5rem 0;'>", unsafe_allow_html=True)
            
#             # Export options
#             output_filename_csv = "Comprehensive_Resume_Details.csv"
#             os.makedirs('output', exist_ok=True)
#             output_path_csv = f'output/{output_filename_csv}'
#             comp_details_df.to_csv(output_path_csv, index=False)

#             with open(output_path_csv, "rb") as file:
#                 st.download_button(
#                     label="📥 Download Comprehensive Details as CSV",
#                     data=file,
#                     file_name=output_filename_csv,
#                     mime="text/csv"
#                 )

#         # ========== NEW UI BLOCK FOR GITHUB LINK EXTRACTION ==========
#         elif analysis_type == "GitHub Links Extraction":
#             st.markdown("<h2>GitHub Profile Link Extraction</h2>", unsafe_allow_html=True)
#             st.info("This feature finds the main GitHub profile link (e.g., github.com/username) from each resume.")


#             if 'github_links_results' not in st.session_state:
#                 with st.spinner('⏳ Extracting GitHub links from resumes...'):
#                     if uploaded_file is not None:
#                         github_df = extract_and_analyze_github_links(uploaded_file)
#                     else:
#                         # Use a temporary file for text input DataFrame
#                         with tempfile.NamedTemporaryFile(delete=False, suffix='.csv') as tmp:
#                             text_input_df.to_csv(tmp.name, index=False)
#                             tmp_path = tmp.name
                        
#                         github_df = extract_and_analyze_github_links(tmp_path)
#                         os.unlink(tmp_path)
                    
#                     st.session_state['github_links_results'] = github_df
            
#             github_df = st.session_state['github_links_results']
            
#             # Display metrics
#             display_metrics(github_df, "GitHub Links Analysis", "Total Resumes")
...
#             # Export options
#             output_filename_csv = "GitHub_Profile_Links.csv"
#             os.makedirs('output', exist_ok=True)
#             output_path_csv = f'output/{output_filename_csv}'
#             github_df.to_csv(output_path_csv, index=False)

#             with open(output_path_csv, "rb") as file:
#                 st.download_button(
#                     label="📥 Download GitHub Links as CSV",
#                     data=file,
#                     file_name=output_filename_csv,
#                     mime="text/csv"
#                 )
...
#         * **GitHub Integration:** View repository counts and extract GitHub links
...
#         3. Select the analysis type (Projects, Skills, Details, etc.)
...
#         You can configure up to 10 different Gemini API keys in the Settings panel to maximize your processing capacity and handle even larger batches of resumes.
#         """)
...
# NOTE: The initialization of ocr_stats is moved to the main() function to ensure it's set before processing begins.
...
def process_scanned_resume(pdf_file):
    ...
    # FIX: Ensure ocr_stats exists before trying to access it
    if 'ocr_stats' not in st.session_state:
        return "Error: OCR stats not initialized in session state."
...
def analyze_text_with_gemini(prompt):
    ...
    for attempt in range(len(valid_keys)):
        ...
        gemini_api_url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-lite:generateContent?key={current_api_key}"
...
def process_resume_with_details(row):
    ...
    if resume_pdf:
        try:
            ...
            # FIX: Add check for failed text extraction
            if not resume_text or len(resume_text.strip()) < 50: # Check for minimal text content
                logging.warning(f"Could not extract sufficient text for user {user_id}")
                return {
                    'User ID': user_id,
                    'Resume Link': resume_link,
                    'Project Titles': 'Failed to extract text from resume.',
                    'Experiences': 'Failed to extract text from resume.',
                    'Links': 'Failed to extract text from resume.',
                    'Repo Count': None
                }
...
def display_metrics(df, title, count_label="Total Resumes"):
    ...
    # Check which columns exist in the DataFrame to determine metric calculation
    # FIX: Correctly calculate processed_count by checking for non-null and non-failed entries
    processed_count = 0
    if not df.empty:
        if 'Project Titles' in df.columns:
            processed_count = df[(df['Project Titles'].notna()) & (~df['Project Titles'].str.contains('Failed', na=True, case=False))].shape[0]
        elif 'Skills' in df.columns:
            processed_count = df[(df['Skills'].notna()) & (~df['Skills'].str.contains('Failed', na=True, case=False))].shape[0]
        elif 'Full Name' in df.columns:
            processed_count = df[(df['Full Name'].notna()) & (~df['Full Name'].str.contains('Failed', na=True, case=False))].shape[0]
        elif 'GitHub Links' in df.columns:
            processed_count = df[(df['GitHub Links'].notna()) & (~df['GitHub Links'].str.contains('Failed', na=True, case=False))].shape[0]
...
user456    https://example.com/resume2.pdf"""
        ...
        if text_input:
            ...
            if not text_input_df.empty:
                st.markdown("<h4>Parsed Input Preview:</h4>", unsafe_allow_html=True)
                st.dataframe(text_input_df, width='stretch') # FIX: Deprecation warning
    ...
    if input_data_ready:
        ...
        if analysis_type == "Project Analysis":
            ...
            # Show the data
            st.dataframe(result_df, width='stretch') # FIX: Deprecation warning
        ...
        # Regular substring matching for other technologies
                ...
                if not matched_df.empty:
                    st.markdown(f"<div class='success-box'><b>✅ Found {matched_count} matches</b> for your selected technologies</div>", unsafe_allow_html=True)
                    st.dataframe(matched_df[['User ID', 'Resume Link', 'Project Titles', 'Experiences', 'Links', 'Repo Count']], width='stretch') # FIX: Deprecation warning
            ...
            # Show the data
            st.dataframe(skills_df, width='stretch') # FIX: Deprecation warning
            ...
            if input_skills:
                ...
                if not matched_skills_df.empty:
                    st.markdown(f"<div class='success-box'><b>✅ Found {matched_count} matches</b> for your selected skills</div>", unsafe_allow_html=True)
                    st.dataframe(matched_skills_df[['User ID', 'Resume Link', 'Skills']], width='stretch') # FIX: Deprecation warning
            ...
            # Show the data
            st.dataframe(details_df, width='stretch') # FIX: Deprecation warning
        ...
        # ===== NEW UI BLOCK FOR COMPREHENSIVE DETAILS =====
            ...
            # Show the data
            st.dataframe(comp_details_df, width='stretch') # FIX: Deprecation warning
        ...
        # ========== NEW UI BLOCK FOR GITHUB LINK EXTRACTION ==========
            ...
            # Show the data
            st.dataframe(github_df, width='stretch') # FIX: Deprecation warning
...
def main():
    ...
    # FIX: Initialize OCR stats in session state at the beginning of the main app flow.
    # This ensures it exists before any processing threads are spawned.
    if 'ocr_stats' not in st.session_state:
        st.session_state.ocr_stats = {
            'scanned_count': 0,
            'ocr_success_count': 0,
    ...
    with st.sidebar:
        ...
        # Navigation
        st.markdown("<h3>Navigation</h3>", unsafe_allow_html=True)
        # FIX: Provide a non-empty label to the radio button to fix the accessibility warning.
        page = st.radio("Navigation", ["Resume Analysis", "About"], label_visibility="collapsed")
